...
class DispersionData:
    ...
    def __init__(
        self, la_data: tuple, max_freq_la: float, ta_data: tuple, max_freq_ta: float
    ):
        """
...
class Material:
    def __init__(self, name: str, d_data: DispersionData, r_data: RelaxationData):
        self.name = name
        self.d_data = d_data
        self.r_data = r_data
...
class EmitSurface:
    def __init__(
        self, p1: Point, p2: Point, temp: float, duration: float, start_time: float
    ):
        self.p1 = p1
        self.p2 = p2
        self.temp = temp
...
class Model:
    def __init__(
        ...
        sensors: List[Sensor],
        cells: list,
        emit_surfaces: List[EmitSurface],
    ):
...
class ModelBuilder:
    ...
    def setNumRuns(self, num_runs: int):
        """The number of different simulations runs to performs with the given
        settings. Each run will be seeded differently and the results of the runs
        will be averaged to produce a single output file.
        """
        if num_runs < 1:
            raise ValueError("The number of runs cannot be less than 1.")
        self.num_runs = num_runs
    ...
    def setSimType(self, sim_type: int, step_interval: int = 0):
        ...
        if self.sim_type == 0 and step_interval > 0:
            raise ValueError(
                "Steady state simulation should have a " "step interval of 0."
            )
        if (self.sim_type == 1 or self.sim_type == 2) and step_interval == 0:
            raise ValueError(
                "Transient and periodic simulations cannot have "
    ...
    def setMeasurements(self, num_measurements: int):
        ...
        if num_measurements <= 10:
            raise ValueError(
                "The number of measurements must be a "
    ...
    def addRawMaterial(self, name: str, d_data: DispersionData, r_data: RelaxationData):
    ...
    def addTriangleCell(
        self, p1: tuple, p2: tuple, p3: tuple, sensorID: int, specularity: float
    ):
        ...
        """
        if specularity > 1.0 or specularity < 0.0:
        ...
        self.cells.append(
            Cell(
                Triangle(Point(p1[0], p1[1]), Point(p2[0], p2[1]), Point(p3[0], p3[1])),
                sensorID,
                specularity,
            )
        )

    def addRectangularCell(
        self, p1: tuple, p2: tuple, sensorID: int, specularity: float
    ):
        ...
        self.addTriangleCell(p1, (p1[0], p2[1]), (p2[0], p1[1]), sensorID, specularity)
        self.addTriangleCell(p2, (p2[0], p1[1]), (p1[0], p2[1]), sensorID, specularity)
    ...
    def addQuadrilateralCell(
        ...
        p2 = (
            p1[0] + dx * math.cos(math.radians(angle)),
            p1[1] + dx * math.sin(math.radians(angle)),
        )
        p3 = (
            p1[0] + dy * math.cos(math.radians(angle + 90)),
    ...
    def addSurface(
        ...
        self.surfaces.append(
            EmitSurface(
                Point(p1[0], p1[1]), Point(p2[0], p2[1]), temp, duration, start_time
            )
        )
    ...
    def __verifySensors(self):
        ...
        for sensor in self.sensors:
            ...
            if not s_mat_valid:
                raise ValueError(
                    "Material {} from sensor {} does not exist"
                    " in the materials list.".format(sensor.material, sensor.ID)
                )
            s_id_valid = False
            for cell in self.cells:
                if sensor.id == cell.sensorID:
                    ...
                    break
            if not s_id_valid:
                raise ValueError("No cells are linked to Sensor {}".format(sensor.id))
    ...
    def __verifyCells(self):
        ...
        for cell in self.cells:
            ...
            if not valid:
                raise ValueError(
                    "A cell has a sensorID of {} but no "
                    "sensor with this ID exists.".format(cell.sensorID)
                )
    ...
    def export(self, filepath: str):
        ...
        model = Model(ss, self.materials, self.sensors, self.cells, self.surfaces)
