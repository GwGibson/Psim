...
void Cell::validate(const Cell& other) const {
    if (cell_.intersects(other.cell_)) { throw IntersectError(this->cell_, other.cell_); }
    if (cell_.contains(other.cell_)) { throw OverlapError(this->cell_, other.cell_); }
    if (other.cell_.contains(cell_)) { throw OverlapError(other.cell_, this->cell_); }
}
...
bool Cell::setEmitSurface(const Line& line, double temp, double duration, double start_time) {
    ...
    for (auto& boundary : boundaries_) {
        if (boundary.addEmitSurface(line, *this, getMaterial(), temp, norm_sign, duration, start_time)) { return true; }
    }
    return false;
}
...
double Cell::getEmitEnergy(double t_eq) const noexcept {
    ...
    for (const auto& surface : getBoundaries()) {
        ...
        emit_energy += std::transform_reduce(std::execution::seq,
            std::cbegin(emit_surfaces),
            std::cend(emit_surfaces),
            0.,
            std::plus{},
            [&](const auto& sub_surface) {
                const auto temp = sub_surface.getTemp();
                const double energy = sub_surface.getSurfaceLine().length * sub_surface.getEmitDuration()
                                      * getMaterial().emitEnergy(temp) / 4.;
                return (t_eq == 0.) ? energy : energy * std::fabs(temp - t_eq);
            });
    }
    return emit_energy;
}
...
void Cell::updateEmitTables() noexcept {
    for (auto& surface : boundaries_) { surface.updateEmitSurfaceTables(); }
}
...
void Cell::findTransitionSurface(Cell& other) {
    ...
    for (const auto& l1 : lines) {// NOLINT
        for (const auto& l2 : other_lines) {// NOLINT
...
bool Cell::setTransitionSurface(const Line& line, Cell& cell) {
    const int norm_sign = (cell_.isClockwise()) ? 1 : -1;
    for (auto& boundary : boundaries_) {
        if (boundary.addTransitionSurface(line, cell, norm_sign)) { return true; }
    }
    return false;
}

std::ostream& operator<<(std::ostream& os, const Cell& cell) {// NOLINT
