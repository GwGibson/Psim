...
[[nodiscard]] static bool isPointOnLine(const Line& line, const Point& p) noexcept;// NOLINT
[[nodiscard]] static bool isPointRightOfLine(const Line& line, const Point& p) noexcept;// NOLINT
[[nodiscard]] static bool doesSegmentCrossLine(const Line& l1, const Line& l2) noexcept;// NOLINT
[[nodiscard]] static double crossProduct(const Point& p1, const Point& p2) noexcept;// NOLINT
[[nodiscard]] static double dotProduct(const Point& p1, const Point& p2) noexcept;// NOLINT
...
bool Line::contains(const Line& other) const noexcept {
    ...
    if (isPointOnLine(*this, p3) && isPointOnLine(*this, p4) && length >= other.length) {
        ...
        return (l1_x_max >= l2_x_max - GEOEPS && l1_x_min <= l2_x_min + GEOEPS && l1_y_max >= l2_y_max - GEOEPS
...
bool Line::intersects(const Line& other) const noexcept {
    return boxesIntersect(boundingBox, other.boundingBox) && doesSegmentCrossLine(*this, other)
...
std::optional<Point> Line::getIntersection(const Line& other) const noexcept {
    ...
    auto equals = [](double a, double b) {// NOLINT
    ...
    auto handle_vert = [&](const auto& pt1, const auto& pt3, const auto& pt4) {
        if (equals(pt3.x, pt4.x)) {// both lines vertical
            ...
            const auto m = getSlope(pt3, pt4);// NOLINT
            const auto b = getIntercept(pt3.x, pt3.y, m);// NOLINT
            poi = { pt1.x, m * pt1.x + b };
        }
    };
    if (intersects(other)) {// If the line segments intersect, find the poi
        ...
        if (equals(t_p1.x, t_p2.x)) {// l1 is a vertical line - needs special consideration
            ...
            if (equals(slope,
                    other.slope)) {// Lines are parallel (by convention, no intersection - will this be a problem?)
                poi = std::nullopt;
            } else {// Only 1 point of intersection -> should be the most common case
                const double x = (other.intercept - intercept) / (slope - other.slope);// NOLINT
...
Point Line::getRandPoint(double r1) const noexcept {// NOLINT
    const double r2 = 1. - r1;// NOLINT
...
[[nodiscard]] std::array<Line, 3> Triangle::lines() const noexcept {// NOLINT
...
bool Triangle::intersects(const Triangle& other) const noexcept {// NOLINT
    const auto intersect = [](const Line& line1, const Line& line2) {
        ...
        if (poi) {
            ...
            if (!(*poi == line1.p1 || *poi == line1.p2 || *poi == line2.p1 || *poi == line2.p2)) { intersects = true; }
        }
        return intersects;
    };
    ...
    for (const auto& line1 : this_lines) {
        if (std::any_of(std::cbegin(other_lines), std::cend(other_lines), [&](const auto& line2) {
...
bool Triangle::contains(const Point& p) const noexcept {// NOLINT
    ...
    const auto u = (d11 * d20 - d01 * d21) / denom;// NOLINT
    const auto v = (d00 * d21 - d01 * d20) / denom;// NOLINT
    if (u >= GEOEPS && u <= 1. + GEOEPS && v >= GEOEPS && v <= 1. + GEOEPS && u + v <= 1. + GEOEPS) {
        if ((area() - getArea(p, p2, p3) + getArea(p1, p, p3) + getArea(p1, p2, p)) < GEOEPS) { return true; }
    }
    return false;
}
...
#else
Point Triangle::getRandPoint(double r1, double r2) const noexcept {// NOLINT
    if (r1 + r2 > 1.) {
        r1 = 1 - r1;
        r2 = 1 - r2;
    }
    const auto x = p1.x + (p2.x - p1.x) * r1 + (p3.x - p1.x) * r2;// NOLINT
    const auto y = p1.y + (p2.y - p1.y) * r1 + (p3.y - p1.y) * r2;// NOLINT
    return { x, y };
}
#endif
bool Triangle::operator==(const Triangle& rhs) const {
    auto contains = [](const auto& t, const auto& p) { return (p == t.p1 || p == t.p2 || p == t.p3); };// NOLINT

    return contains(rhs, p1) && contains(rhs, p2) && contains(rhs, p3);
}

double Triangle::area() const noexcept {// NOLINT
    const auto& [l1, l2, l3] = lines();
    const auto a = l1.length;// NOLINT
    const auto b = l2.length;// NOLINT
    const auto c = l3.length;// NOLINT
    const auto p = (a + b + c) / 2.;// NOLINT
...
bool Triangle::contains(const Triangle& other) const noexcept {// NOLINT
...
PointPair findBoundingBox(const Line& line) noexcept {
    ...
    auto tr_x = p2.x;
    auto tr_y = p2.y;
    if (p2.x <= p1.x) { std::swap(bl_x, tr_x); }
    if (p2.y <= p1.y) { std::swap(bl_y, tr_y); }
...
bool isPointOnLine(const Line& line, const Point& p) noexcept {// NOLINT
    const auto& [p1, p2] = line.getPoints();
    return fabs(crossProduct({ p2.x - p1.x, p2.y - p1.y }, { p.x - p1.x, p.y - p1.y })) < GEOEPS;
}

bool isPointRightOfLine(const Line& line, const Point& p) noexcept {// NOLINT
    const auto& [p1, p2] = line.getPoints();
    return crossProduct({ p2.x - p1.x, p2.y - p1.y }, { p.x - p1.x, p.y - p1.y }) < 0.;
}

bool doesSegmentCrossLine(const Line& l1, const Line& l2) noexcept {// NOLINT
    return isPointOnLine(l1, l2.p1) || isPointOnLine(l1, l2.p2)
           || ((isPointRightOfLine(l1, l2.p1) ^ isPointRightOfLine(l1, l2.p2)) != 0);
}

double crossProduct(const Point& p1, const Point& p2) noexcept {// NOLINT
    return p1.x * p2.y - p2.x * p1.y;
}

double dotProduct(const Point& p1, const Point& p2) noexcept {// NOLINT
    return p1.x * p2.x + p1.y * p2.y;
}

double getLineLength(const Line& line) noexcept {
    const auto p1 = line.p1;// NOLINT
    const auto p2 = line.p2;// NOLINT
...
LineError::LineError(Geometry::Line line)
    : line_{ std::move(line) } {
